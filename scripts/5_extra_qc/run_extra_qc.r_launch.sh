...
tail -n +2 "$SAMPLES_TSV" | while IFS=$'\t' read -r sample_id fivebase_frag fivebase_pileup ont_fraglen ont_cpg_density ont_calls; do
  ...
  sbatch \
    ...
    --wrap="Rscript '$PROJECT_DIR/scripts/5_extra_qc/coverage_breadth_overlap.r' '$OUT_DIR/fivebase_sites.parquet' '$OUT_DIR/ont_sites.parquet' '$sample_id' '$OUT_DIR/coverage_breadth_overlap.tsv' && \
              ...
              Rscript '$PROJECT_DIR/scripts/5_extra_qc/methylation_by_fragment_length.r' '$fivebase_frag' '$ont_cpg_density' '$sample_id' '$OUT_DIR/methylation_by_fragment_length.tsv' '$PLOT_DIR/methylation_by_fragment_length.png'"
done
